...
1. `docs/plans/CURRICULUM_V0_5_PLAN.md` §3 — the Strand B framing, apex unit list, supporting units, and shipped foundations to reuse.
2. `docs/plans/PILOT_PLAN.md` — current shipped count (66 units after Wave 5), v0.x retrospective.
3. `docs/plans/WAVE_3_PLAN.md` and `docs/plans/WAVE_4_PLAN.md` — depth-completion plans (for context on how foundations shipped).
4. `docs/pilot-lessons.md` — consolidated production lessons including the LM editorial pass findings.
5. `docs/specs/UNIT_SPEC.md` — frontmatter, tier markers, citation syntax (note §11 Style: the six Master-prose failure modes).
6. `docs/specs/QUALITY_RUBRIC.md` — the 27-point automated checklist + §M Master prose standard.
7. `docs/catalogs/CONCEPT_CATALOG.md` — find each concept entry; add parent entries first if missing (e.g., `symplectic-geometry.symplectic-manifold` namespace).
...
After the apex queue, produce supporting units identified in `docs/plans/CURRICULUM_V0_5_PLAN.md` §3 (integrable systems, Darboux's theorem, action-angle coordinates, etc.) as a Strand B Wave 2 batch. Aim for 8–10 supporting units.
...
3. Update `docs/catalogs/CONCEPT_CATALOG.md` — add the new concept entry. Add parent namespace entries if missing (e.g., `symplectic-geometry.symplectic-manifold`, `symplectic-geometry.hamiltonian-system`, `symplectic-geometry.moment-map`).
...
6. Append a paragraph to `docs/pilot-lessons.md` per unit on what was non-obvious to produce.
7. After the apex queue is complete, run `./.venv/bin/python scripts/validate_all.py` and confirm green; then `npm run build` from `site/` (in the codex repo) to confirm rendering.
...
Begin with `05.01.01` Symplectic vector space. Read `docs/plans/CURRICULUM_V0_5_PLAN.md` §3 first.
