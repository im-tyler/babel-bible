...
Because Codex's existing 24-unit `03.12-homotopy/` corpus and the
...
**Theorem layer.** ~80% of May's named theorems map to Codex units (the
...
2. **`03.12.25` Simplicial sets and geometric realization.** Definition
   ...
   adjunction**. Three-tier; ~1800 words. Foundational stub at Codex
...
- `h𝒞` for the homotopy category of any category `𝒞` equipped with a
  homotopy relation. Codex's existing homotopy-category usage in
...
- $Mf$ for the reduced mapping cylinder, $Nf$ for the mapping path space,
  $Cf$ for the homotopy cofiber, $Ff$ for the homotopy fiber. Codex's
...
- $S_*X$ for the total singular complex (a simplicial set),
  ...
  functorial CW approximation. None of these appear in Codex yet.
...
- **The simplicial-objects machinery beyond the foundational stub.**
  ...
  required to make the May framework legible in Codex.
...
- **Figures.** May uses many small commutative-diagram pictures (the
  ...
  load-bearing for intuition. Codex's figure infrastructure is the same
...
- ≥95% of May's named theorems in Chs 1–22 map to Codex units (currently
...
- ≥90% of May's worked examples are reproduced in some Codex unit
