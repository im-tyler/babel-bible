...
ISBN 0-201-09394-4. Citations in Codex units already point at "Atiyah —
...
1. **Vector bundles via continuous functors.** Ch. I §1.2 packages
   ...
   the literature and is exactly the functoriality the Codex unit
...
- Raoul Bott, Loring Tu, *Differential Forms in Algebraic Topology*
  ...
  peer; not a K-theory text per se but the Codex `03.08.05` unit is
  Bott-Tu-anchored.
...
| §1.4 | Vector bundles on compact spaces (homotopy invariance, stable inverse, classification by $[X, BU(n)]$) | `03.05.02`, `03.08.04-classifying-space` | △ | Classification is in `03.08.04`; the four compactness lemmas (1.4.1 complement, 1.4.3 homotopy invariance, 1.4.7 stable inverse, 1.4.9 finite-dim classification) are *implicit* in Codex but not theorem-numbered. **Add a §"Compactness lemmas" block to `03.05.02` Master.** |
...
| §2.5 | Computations of $K^*(X)$ for $X = S^n$, $\mathbb{C}P^n$, $T^n$, Stiefel manifolds, $\mathbb{R}P^n$ (the latter via $\psi^k$ obstruction) | — | ✗ | **Gap.** Worked computations are absent from the Codex. The $\mathbb{C}P^n$ computation $K^*(\mathbb{C}P^n) = \mathbb{Z}[H]/(H-1)^{n+1}$ is a load-bearing example used throughout the index-theorem chapter. |
...
existing Codex units. The Ch. I foundations (§1.1–§1.5) are ~80%
...
$\widetilde K(X)$ for reduced; the Codex (per `03.08.01`) writes $K^0(X)$
...
for the Bott class; the Codex should adopt the same. Atiyah's $\lambda_E$
...
- ≥95% of Atiyah's named theorems (compactness lemmas of §1.4,
  ...
  identities §3.2, $J(X)$ exactness §3.3) map to Codex units. Current:
...
- ≥90% of Atiyah's worked computations ($K^*(S^n)$, $K^*(\mathbb{C}P^n)$,
  ...
  direct Codex unit or are referenced from a unit that covers them
  (closed by `03.08.09`).
...
- The $\lambda$-ring / Adams-operations story (Atiyah Ch. III) has at
  least one dedicated Codex unit (closed by `03.08.02` + `03.08.08`).
