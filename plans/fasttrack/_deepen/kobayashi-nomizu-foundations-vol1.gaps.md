...
Free `03.02.*` slots verified occupied through `03.02.38`; next free is
`03.02.39`. The entire `03.05.00–03.05.23` band is occupied (both the
...
### 1. `03.02.39` — Cartan–Ambrose–Hicks theorem (intrinsic affine/Riemannian rigidity)
...
- **prereqs:** `03.02.27` (Levi-Civita / exp), `03.02.05` (sectional/Ricci/scalar
  curvature), `03.05.16` (holonomy group), `03.02.32` (Riemannian Hopf–Rinow,
...
- **why a gap, not COVERED:** the only CAH statement in `content/` is
  `03.02.14`'s **Theorem 5 "Cartan–Ambrose–Hicks theorem for submanifolds"**
  ...
  determined by sectional curvatures" (`03.02.05`) and "locally symmetric ⇔
...
- **Freestanding "Lie derivative of tensor fields / Lie bracket of vector fields"
  unit.** The original audit's priority-0 item 2 (`03.02.03`). The Lie bracket of
  vector fields is *defined and used* (`03.02.04` Frobenius: the bracket as the
  ...
  `03.02.07` (Killing). A dedicated unit consolidating $\mathcal L_X$ on general
...
|---|---|---|
| Smooth manifold, atlas, charts | `03.02.01`, `03.02.02`, `03.02.03` | topological-manifold + smooth-structure-atlases + smooth-maps now a full chain (audit saw only one unit) |
| Frobenius theorem (audit item 3, `03.02.04`) | `03.02.04` | dedicated unit; involutive ⇔ integrable, bracket/local-frame proof |
| Lie bracket of vector fields | `03.02.04` | defined + worked: bracket as involutivity obstruction |
| Lie derivative (Cartan magic formula) | `03.04.04`, `03.02.07` | $\mathcal L_X=d\iota_X+\iota_X d$ stated; $\mathcal L_X g$ in Killing unit |
...
| General fibre bundle (audit item 5, `03.05.00`) | `03.05.00` (`05-fibre-bundles/`) | dedicated general-fibre-bundle unit |
| Principal $G$-bundle | `03.05.01` | shipped |
| Vertical subbundle + fundamental vector fields (audit item 6, `03.05.06`) | `03.05.06` (`05-fibre-bundles/`) | dedicated unit (unblocked the `03.05.07` Lean gap) |
| Connection on principal bundle (1-form + horizontal dist.) | `03.05.07` | shipped, KN-I §II anchor |
| Horizontal lift + parallel transport (audit item 7, `03.05.11`) | `03.05.11` (`05-fibre-bundles/`) | dedicated unit |
| Curvature 2-form, Cartan structural eqn $\Omega=d\omega+\tfrac12[\omega,\omega]$ | `03.05.09` | shipped, KN-I §III anchor |
| Reduction of structure group + of connection (audit item 8, `03.05.12`) | `03.05.12` (`05-fibre-bundles/`) | dedicated general-reduction unit; `03.05.03` is the O(n) example |
| Associated bundle $P\times_G F$ + induced connection (audit item 9, `03.05.13`) | `03.05.13` (`05-fibre-bundles/`) | dedicated unit |
| Connections on $P$ ↔ covariant derivatives on assoc. vector bundles | `03.05.04`, `03.05.13` | equivalence stated in `03.05.04` (Leibniz/$\mathbb R$-linear characterisation) |
| Frame bundle $L(M)$, linear connection, soldering form (audit items 11, `03.05.15`) | `03.05.15` (`05-fibre-bundles/`) | dedicated unit; general GL(n) frame bundle + canonical $\theta$ |
| Torsion tensor + two Cartan structural equations (audit item 10, `03.05.14`) | `03.05.14` (`05-fibre-bundles/`) | dedicated unit; both structural eqns |
| First + second Bianchi identities (audit item 20) | `03.05.14` | **both** stated as "Theorem (Bianchi identities)" + algebraic-Bianchi exercise |
| Geodesic of affine connection / auto-parallel | `03.02.27`, `13.02.02` | Levi-Civita unit + geodesics/parallel-transport unit |
| Exponential map $\exp_p$, normal coordinates, completeness (audit item 12) | `03.02.27`, `03.02.32` | Levi-Civita/exp unit (do Carmo Ch. 7 completeness locator) + Hopf–Rinow |
| Levi-Civita connection (existence + uniqueness) (audit item 12) | `03.02.27` | dedicated unit, "fundamental lemma Thm 3.6" |
| Hopf–Rinow theorem (audit item 12) | `03.02.32` | dedicated Riemannian Hopf–Rinow unit |
| Sectional / Ricci / scalar curvature (audit item 13, `03.02.05`) | `03.02.05` | dedicated unit |
| Constant-curvature spaces + Killing–Hopf (audit item 14, `03.02.06`) | `03.02.06` | dedicated unit; $S^n/\mathbb R^n/\mathbb H^n$ classification |
| Killing fields / infinitesimal isometries (audit item 18, `03.02.07`) | `03.02.07` | dedicated unit; $\mathcal L_X g=0$, Lie algebra of Killing fields |
| Myers–Steenrod (audit item 19, `03.02.08`) | `03.02.08` | dedicated unit; isometry group is a Lie group |
| Holonomy group + restricted holonomy (audit item 15, `03.05.16`) | `03.05.16` (`05-fibre-bundles/`) | dedicated unit |
| Ambrose–Singer theorem (audit item 16, `03.05.17`) | `03.05.17` (`05-fibre-bundles/`) | dedicated unit |
| Holonomy reduction theorem (audit item 17, `03.05.18`) | `03.05.18` (`05-fibre-bundles/`) | dedicated unit |
...
| CAH for submanifolds (extrinsic / fundamental theorem) | `03.02.14` | "Theorem 5 (Cartan–Ambrose–Hicks for submanifolds)" — *distinct* from the intrinsic gap #1 |

**Notes on judgement calls.**
- The original audit ran in REDUCED mode (no PDF, working from the TOC) and
  predates the entire `03.02.05–03.02.08`, `03.02.27`, `03.02.32`,
  `03.05.06`, `03.05.11–03.05.18` production wave. Every "✗ Gap" row in its §2
  table has since been filled. I re-verified each by `find`/`grep` against the
  live tree rather than trusting the table.
- `03.02.14`'s CAH-for-submanifolds is **not** double-counted as covering gap #1:
...
- "Curvature determined by sectional curvatures" (`03.02.05` Thm) and
