...
`48.02.26`, `48.02.29`, `48.02.32`, `48.02.33`, `48.02.34`.

---

## Genuine gaps (6)

### 1. `48.02.26` — Harmonic maps: energy, tension field, harmonic-map equation
...
- **prereqs:** `48.02.05` (sectional/Ricci/scalar curvature), `48.02.27`
...
### 2. `48.02.29` — Harmonic-map heat flow and the Eells–Sampson theorem
- **spec:** $\partial_t u=\tau(u)$; for $M$ compact, $N$ compact of nonpositive
  ...
  sub-convergence.
- **prereqs:** `48.02.26` (harmonic maps, gap #1), `48.02.15` (Bochner technique)
- **chapter dir:** `content/03-differential-geometry/02-manifolds/`
- **originator:** Eells–Sampson 1964. (Note: the harmonic-map Bochner identity is
  *not* the same as the Bochner/Weitzenböck content in `48.02.15`, which is for
  forms/spinors — it is a genuinely new identity for maps, so it belongs here
  rather than as a separate unit.)

### 3. `48.02.32` — Riemannian Hopf–Rinow theorem
...
- **prereqs:** `48.02.27` (Levi-Civita/exp), `13.02.02` (geodesics/parallel transport)
- **chapter dir:** `content/03-differential-geometry/02-manifolds/`
- **originator:** Hopf–Rinow 1931 (*Comment. Math. Helv.* 3, 209–225).
- **why a gap, not COVERED:** the theorem is *invoked* in `48.02.07`
  (Killing-field completeness), `48.02.06`, and the Jacobi unit, but the only unit
  that *states/proves* a Hopf–Rinow theorem is `48.02.17`, which is the
...
### 4. `48.02.33` — Yamabe problem and the conformal Laplacian
- **spec:** Conformal class $g\mapsto e^{2f}g$; conformal Laplacian
  ...
  theorem.
- **prereqs:** `48.02.05` (scalar curvature), `48.02.16` (Weyl tensor / conformally
...
### 5. `48.02.34` — Toponogov triangle comparison theorem
...
- **prereqs:** `48.02.19` (Jacobi fields / index form, where Rauch is stated),
  `48.02.06` (constant-curvature model spaces)
- **chapter dir:** `content/03-differential-geometry/02-manifolds/`
- **originator:** Toponogov 1959 (*Uspekhi Mat. Nauk* 14, 87–130).
- **why a gap:** **zero hits for "toponogov" in `content/`** (only in plan files).
  Rauch is *stated* in `48.02.19`; Toponogov (the global triangle version, which
...
- **prereqs:** `48.02.19` (second variation / index form), `48.02.32` (Hopf–Rinow,
...
|---|---|---|
| Levi-Civita connection, Christoffel symbols, metric compatibility | `48.02.27`, `13.02.02`, `48.05.15` | `48.02.27` defines Levi-Civita + exp; `13.02.02` gives the Christoffel formula and $\nabla g=0$ explicitly |
| Exponential map, normal coordinates, Gauss lemma | `48.02.27`, `48.02.06` | exp map and injectivity-radius/normal-coordinate use throughout `48.02.06` |
...
| Sectional / Ricci / scalar curvature | `48.02.05` | dedicated unit; the $K(\sigma)$ formula, Ricci as trace, scalar as double trace |
| Jacobi fields, conjugate points, Morse index theorem | `48.02.19` | dedicated unit; Jacobi equation, index form, index theorem |
| Second variation of energy / index form | `48.02.19` | "Definition (index form)" = second variation $E_{**}$ |
| Bonnet–Myers theorem | `48.02.19` (infinitesimal), `48.02.06` (global) | exercise derives the diameter bound; applied globally in `48.02.06` |
| Cartan–Hadamard theorem | `48.02.06` (Thm 1), `07.04.07` | full statement + $\exp_p$ diffeomorphism; reused for NPC symmetric spaces |
| Rauch comparison theorem | `48.02.19` | stated as the index-form comparison (curvature bound ⇒ conjugate-point shift) |
| Bishop–Gromov / volume comparison | `48.02.05` (Thm 4) | scalar-curvature volume comparison against constant-curvature model |
| Spaces of nonpositive curvature (Cartan–Hadamard manifolds) | `07.04.07`, `48.02.06` | NPC symmetric spaces diffeomorphic to $\mathbb R^n$ via Cartan–Hadamard |
| Bochner technique / Weitzenböck / vanishing theorems (forms & spinors) | `48.02.15`, `03.09` chain | dedicated Bochner unit; positive-Ricci kills harmonic 1-forms; Lichnerowicz |
...
| Morse on loop spaces / Bott periodicity (Jost Ch. 8) | `48.02.19`, `03.15.*` | index theorem → path-space CW structure → Bott periodicity |
...
- *Riemannian metric tensor / isometry as a standalone unit* — not promoted: the
  metric tensor is defined and used across `48.02.05`, `48.02.06`, `13.02.02`,
  `48.05.20`; a dedicated "definition of $g$" unit would be marginal. COVERED in
  effect.
