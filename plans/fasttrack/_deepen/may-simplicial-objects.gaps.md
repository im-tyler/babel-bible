...
- **prereqs:** `48.12.39` (simplicial-group-wbar-classifying-functor),
...
- **why a gap:** "twisted cartesian product" / "twisting function" appears *only* as a
  ...
  string "twisting function" returns zero unit bodies. `48.12.39` builds $\overline{W}G$
...
- **why a gap:** the *facts* are cited as established — `03.12.25` states "the simplicial
  ...
  lives in `03.12.52` and the topological fibration LES in `48.12.28` (Puppe), but the
...
|---|---|---|
| Simplicial category $\Delta$, face/degeneracy, simplicial identities, normal form (§1) | `48.12.24` simplicial-set-delta | "These satisfy the **simplicial identities**…"; full functor definition $\Delta^{op}\to\mathbf{Set}$ |
| Simplicial set / simplicial object in a category (§2) | `48.12.24`, `03.12.25`, `01.02.35` | def of $\mathbf{sSet}=\mathbf{Fun}(\Delta^{op},\mathbf{Set})$; `01.02.35` does general simplicial object in additive $\mathcal{A}$ |
| Eilenberg-Zilber **lemma** (unique non-degenerate decomposition) (§2) | `48.12.24`, `03.12.25` | "Every simplex $x$… unique $x=\alpha^*\tilde x$, $\tilde x$ non-degenerate" |
| Kan complex / extension condition (§3) | `03.12.25`, `03.12.33`, `03.12.40` | horn-filling def; $\mathrm{Sing}(X)$ is Kan (proof in `03.12.25`) |
| Simplicial homotopy ($\sim$ on $K_n$) as equivalence relation (§3) | partial: `03.12.25`, `48.12.30` | $\sim$ used in minimal-complex def; **construction of $\pi_n$ from it is Gap 3** |
...
| Minimal complex / minimal fibration (§9-10) | `48.12.30` minimal-complex-minimal-fibration | existence of minimal subcomplex as deformation retract; factorisation theorem; uniqueness |
...
| Comparison $|\mathrm{Sing}(X)|\to X$ weak equivalence (§16) | `03.12.25`, `48.12.26` (functorial-cw-approximation $\Gamma X=|S_*X|$) | proof sketch citing May 16.1 |
| Simplicial group, every simplicial group is Kan (§4, §17) | `48.12.39` simplicial-group-wbar-classifying-functor | def $G:\Delta^{op}\to\mathbf{Grp}$; horn-filling via multiplication |
| Moore / normalised complex of a simplicial group (§22) | `48.12.39`, `01.02.35` | $N(G)_n=\bigcap\ker d_i$, $\pi_n(|G|)=H_n(N(G))$ |
| Classifying $\overline{W}G$, $BG$, $WG$, loop-group $G(X)$ (§21, §25-26) | `48.12.39`, `03.12.25` | $\overline{W}(G)$, $G\simeq\Omega|\overline{W}G|$, Kan loop group $G(X)$, $|N(G)|=BG$ |
...
| Bisimplicial sets / diagonal / realisation lemma | `48.12.36` bisimplicial-set-diagonal-realisation-lemma | used in $\overline{W}$-preserves-weak-equivalences proof |
