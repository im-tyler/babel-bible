...
`48.05.19`, cross-linked from `06.05.02`). Riemann's existence theorem / abundance of
...
- **prereqs:** `06.03.03` (uniformisation theorem — potential-theoretic statement this complements), `48.02.05` (sectional/Ricci/scalar curvature), `48.02.06` (constant-curvature spaces / Killing-Hopf)
...
- **prereqs:** `06.01.24` (Dirichlet problem / Perron — elliptic-PDE solvability on a surface domain), `06.01.11` (harmonic functions in the plane / Laplacian), `48.02.05` (Gaussian/Ricci curvature)
- **chapter dir:** `content/06-riemann-surfaces/03-riemann-surfaces/` · frontmatter `section: riemann-surfaces`, `chapter: riemann-surfaces`
...
- **Narasimhan-Seshadri (stable bundles ↔ unitary reps)** (brief item) — COVERED `04.10.06-moduli-vector-bundles-curve-slope-stability` (full statement + Atiyah-Bott/Donaldson moment-map proof outline), restated `48.05.19-holomorphic-vector-bundle`, cross-linked `06.05.02`.
- **Riemann mapping theorem / Dirichlet-principle route** (brief item) — COVERED `06.01.06-riemann-mapping-theorem` (Dirichlet-principle / Koebe history) + `06.01.24-dirichlet-problem-perron`. (Plateau/Douglas-Radó is not Donaldson-central and lives adjacently in `48.02.26`/`48.02.29` harmonic-map theory.)
