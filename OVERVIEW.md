...
| 2026-06-17 | Finalization program authorised. Eight scope decisions recorded per `plans/expansion/FINALIZATION_PLAN.md` §4: **S1** carve §48 as Classical & Riemannian Geometry (§03 stays modern/gauge); **S2** keep §08-stat-mech and §11-stat-mech-physics separate (distinct audiences); **S3** §02.09-complex kept in place as the elementary complex-numbers chapter with a cross-ref to §06.01 (migration rejected — unit is legitimately placed in analysis); **S4** new §51-linguistics section (scientific language study); **S5** new §52-economics section (structured micro/macro/econometrics); **S6** condensed-matter physics added as a chapter under §12-quantum; **S7** Law / Political Science / Business / standalone Neuroscience / Materials / standalone Astrophysics deferred to v2 (recorded as explicit scope exclusions); **S8** no further Tier-B spines | Turns the "discretionary curation" remainder into an executable program with a status table; findability criterion (§0.1 of FINALIZATION_PLAN) is now the gate for v1-final |
| 2026-06-17 | Directory names declared cosmetic and load-bearing work declared additive | Verified `validate_unit.py`, `build_lenses.py`, and the site collection all key off frontmatter `section`/`chapter` and unit IDs, never dir names; the findability fix is therefore new chapters + cross-refs + new units, not dir renames or migrations. Avoids the content-loss risk of subtractive moves. |
