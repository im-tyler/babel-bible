...
export async function loader({ params }: { params: { slug: string } }) {
  const all = await getCollection("philosophy");
...
export function head({ data }: { data: any }) {
  return {
    ...
    description: data.chapter.subtitle ?? `Babel Bible philosophy, chapter ${data.chapter.order}.`,
  };
}

export default function PhilosophyChapter({ data }: { data: any }) {
  const c = data.chapter;
  return (
    <article class="unit-page">
      <header class="unit-header">
        <p class="unit-breadcrumb">
          <a href="/philosophy">Philosophy</a> · Chapter {c.order}
        </p>
        <h1>{c.title}</h1>
      ...
      <nav class="page-narrow" aria-label="Chapter navigation">
        <div class="hub-grid">
          {data.prev ? (
            <a href={`/philosophy/${data.prev.slug}`} class="hub-card">
          ...
          {data.next ? (
            <a href={`/philosophy/${data.next.slug}`} class="hub-card">
