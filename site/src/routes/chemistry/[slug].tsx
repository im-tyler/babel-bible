...
export async function loader({ params }: { params: { slug: string } }) {
  const all = await getCollection("chemistry");
...
export function head({ data }: { data: any }) {
  return {
    title: `${data.essay.title} | Chemistry | Babel Bible`,
    description: data.essay.subtitle ?? `Chemistry essay ${data.essay.order}.`,
  };
}

export default function ChemistryEssay({ data }: { data: any }) {
  const e = data.essay;
  return (
    <article class="unit-page">
      <header class="unit-header">
        <p class="unit-breadcrumb">
          <a href="/chemistry">Chemistry</a> · Essay {e.order}
          {e.id && <> · <code>{e.id}</code></>}
        </p>
        <h1>{e.title}</h1>
      ...
      <nav class="page-narrow" aria-label="Essay navigation">
        <div class="hub-grid">
          {data.prev ? (
            <a href={`/chemistry/${data.prev.slug}`} class="hub-card">
          ...
          {data.next ? (
            <a href={`/chemistry/${data.next.slug}`} class="hub-card">
