...
prerequisites: [25.03.01, 25.02.01, 25.07.01]
tier_anchors:
...
![Diagram of a simple neural network with 3 input nodes, a hidden layer of 4 nodes, and 2 output nodes. Arrows connect every node in one layer to every node in the next, representing weights. Labels show how each node computes a weighted sum plus bias, then applies an activation function.](/img/25.09.01-neural-network-placeholder.svg)
