...
- **Modular forms on $\mathrm{SL}_2(\mathbb{Z})$ [21.04.01]** — the modular curve $X(1) = \mathrm{SL}_2(\mathbb{Z}) \backslash \mathbb{H}^*$ is a compact Riemann surface of genus $0$, isomorphic to $\mathbb{P}^1$ via the $j$-invariant. Modular forms are sections of line bundles on $X(1)$, and the dimension formula for the space $M_k$ is the Riemann-Roch theorem for the bundle $\mathcal{L}_k$ in the orbifold setting accounting for the two elliptic points $i$ and $\rho$.
