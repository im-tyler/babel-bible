...
![Diagram showing the TCP/IP layer stack. Two hosts each have five layers stacked vertically (Application, Transport, Network, Link, Physical). A vertical arrow passes through each layer on the left host, crosses the network through routers at the Network layer, and arrives at the right host. Each layer on the left communicates logically with the same layer on the right.](/img/25.06.01-protocol-stack-placeholder.svg)

![Packet encapsulation diagram showing an HTTP message wrapped in a TCP segment, wrapped in an IP datagram, wrapped in an Ethernet frame. Each layer adds its own header.](/img/25.06.01-packet-encapsulation-placeholder.svg)
