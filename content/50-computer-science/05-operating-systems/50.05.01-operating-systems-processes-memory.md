...
prerequisites: [50.01.01, 50.04.01]
tier_anchors:
...
![Diagram showing how virtual memory maps process address spaces to physical memory. Three processes each have their own virtual address space (shown as colored rectangles). Arrows connect virtual pages to physical page frames via a page table. Some virtual pages point to disk storage (swap space) when physical memory is full.](/img/50.05.01-virtual-memory-placeholder.svg)

![Timeline showing round-robin scheduling with three processes P1, P2, P3. Each gets a time quantum Q in turn. P1 runs for Q, then P2 runs for Q, then P3 runs for Q, then back to P1. Context switch overhead is shown as small gaps between process executions.](/img/50.05.01-round-robin-scheduling-placeholder.svg)
