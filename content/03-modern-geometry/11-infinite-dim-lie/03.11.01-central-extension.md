...
**Synthesis.** Central extensions are the cohomological bridge between projective and honest representations: equivalence classes of one-dimensional central extensions of $\mathfrak{g}$ are classified by the second Lie-algebra cohomology group $H^2(\mathfrak{g}; K)$, a 2-cocycle $\omega$ modifies the bracket by a correction term while preserving the Jacobi identity, changing the linear splitting of the extension changes the cocycle by a coboundary, and the Virasoro algebra's Gelfand-Fuchs cocycle $\omega(\ell_m, \ell_n) = \frac{m^3 - m}{12}\delta_{m+n,0}$ is the universal central extension of the Witt algebra. In physics, projective representations of symmetry groups (quantum states defined only up to phase) lift to genuine representations of a centrally extended algebra, making central extensions the algebraic mechanism behind anomalies, the central charge in conformal field theory [03.10.02], and the level in affine Kac-Moody representation theory.
